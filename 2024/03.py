...
class Instr:
    def __init__(self, pattern, cmd: Commands):
        self.pattern = pattern
        self.cmd = cmd

    def eval(self, text):
        first_inst = re.search(self.pattern, text)
        self.match = first_inst

        self.n1 = self.n2 = None

        if first_inst is not None:
            self.start = first_inst.start()
            self.end = first_inst.end()

            if self.cmd == Commands.MUL:
                self.n1 = int(first_inst.groups()[0])
                self.n2 = int(first_inst.groups()[1])
        else:
            self.start = math.inf
            self.end = math.inf

    def __str__(self):
        return f"Cmd {self.cmd.name} (Start: {self.start}, End: {self.end})"
...
def get_next_instr(text):
    mul_pattern = r"mul\(([0-9]{1,3}),([0-9]{1,3})\)"
    do_pattern = r"do\(\)"
    dont_pattern = r"don't\(\)"

    patterns = [
        Instr(mul_pattern, Commands.MUL),
        Instr(do_pattern, Commands.DO),
        Instr(dont_pattern, Commands.DONT),
    ]

    for pattern in patterns:
        pattern.eval(text)

    sorted_patterns = sorted(patterns, key=lambda inst: inst.start)
    print(sorted_patterns)
    winner = sorted_patterns[0]

    # End of case. No matches found
    if winner.start == math.inf:
        return None

    cmd = winner.cmd
    new_text = text[winner.end:]
    n1, n2 = winner.n1, winner.n2

    return cmd, new_text, n1, n2
...
def f(x: str):
    ...
    # Go in a loop until nothing is left
    while (instr := get_next_instr(text)):
        cmd, text, n1, n2 = instr
        if cmd == Commands.DO:
            do = True
...
print(f("xmul(2,4)%&mul[3,7]!@^do_not_mul(5,5)+mul(32,64]then(mul(11,8)mul(8,5))"))
...
new = """xmul(2,4)&mul[3,7]!^don't()mul(5,5)mul(1331,8)undo()mul(8,5))
"""
print(f(new))
