...
spelled = ["alphabet", "one", "two", 'three', 'four', 'five', 'six', 'seven', 'eight', 'nine']
...
def parse_line(line):
    ...
    for i, number in enumerate(spelled):
        n_i = line.find(number)
        if (n_i != -1):
            ints_i.append(n_i)
            ints.append(i)
        n_i = line.rfind(number)
        if (n_i != -1):
            ints_i.append(n_i)
            ints.append(i)
    ...
    try:
        first = ints_i.index(min(ints_i))
        last = ints_i.index(max(ints_i))
    except:
        print(ints_i, line, ints)
        raise

    if (line == "jprxbcghdgxhk9x"):
        print(ints_i, line, ints, first, last, ints[first], ints[last])
    # print(ints_i)
    # print(ints)
    
    x, y = ints[first], ints[last]
...
def f(x):
    ...
    for line in x.splitlines():
        ...
        line_calib = int(f"{first}{last}")
        if line == "jprxbcghdgxhk9x": print(line_calib)
        sum += line_calib
    return sum
