...
def parse_inputs(x:str):
    ...
    for section in rest:
        ...
        for line in section.splitlines()[1:]:
            # print(line, section)
            section_map.append(list(map(int, line.split(" "))))
        all_maps.append(section_map)
...
def overlaps(x, y):
    ...
    if y1 <= x1 and x1 < y2:
        return True
    if y1 <= (x2 - 1) and (x2 - 1) < y2:
        return True
    if x1 <= y1 and y1 < x2:
        return True
    if x1 <= (y2 - 1) and (y2 - 1) < x2:
        return True
    return False

def in_range(num, ranges):
    if ranges[0] <= num and num < ranges[1]:
        return True
    return False
...
def find_range_overlap(x, y):
    x1, x2 = x
    out_start, out_end = y
    if in_range(x1, y) and in_range(x2, y):
        return x
    elif in_range(x1, y):
        remaining = out_end - x1
        return (x1, x1 + remaining)
    elif in_range(x2, y):
        remaining = x2 - out_start
        return (x2 - remaining, x2)
    else:
        return y

def f(x):
    seeds, maps = parse_inputs(x)
    # print(seeds, maps)
    maps.reverse()
    goals = [(0, 10000000)]
    for paragraph in maps:
        next_goals = set()
        streak_kept = True
        for out_start, in_start, num in paragraph:
            for goal in goals:
                if overlaps(goal, [out_start, out_start + num]):
                    start_offset, range_overlap = find_range_overlap(goal, [out_start, out_start + num])
                    next_goals.add((in_start + start_offset, in_start + start_offset + range_overlap))
                    streak_kept = False
        for goal in goals:
            for out_start, in_start, num in paragraph:
                if overlaps(goal, [out_start, out_start + num]):
    ...
    for i in range(len(seeds) // 2):
        in_start, num = seeds[i * 2], seeds[i*2 + 1]
        for goal in goals:
            if (overlaps(goal, [in_start, in_start + num])):
                next_goals.add((in_start, in_start + num))
    goals = next_goals
...
def traverse(x, maps):
    for map in maps:
        touched = False
        for line in map:
            outstart, instart, num = line
            if (overlaps((x, x + 1), (instart, instart + num))):
                next_x = outstart + x - instart
                touched = True
        if not touched:
            next_x = x
        x = next_x
...
def traverse_reverse(x, maps, seeds):
    import copy
    ...
    for map in maps:
        ...
        for line in map:
            outstart, instart, num = line
            if (in_range(x, (outstart, outstart + num))):
                next_x = instart + x - outstart
                # print(outstart, instart, num)
                touched = True
        if not touched:
            next_x = x
        # print(next_x)
...
def new(x):
    # seeds, maps = parse_inputs(x)
    # min_val = None
    # for i in range(3683286274, 3683286274 + 4020916803, 100000):
    #     val = traverse(i, maps)
    #     if min_val == None or min(val, min_val) == val:
    #         min_val = val
    # return min_val
    ...
    # 3974786000
    # 3974785726
    # 3974775726, 252453584
    # 3974775626, 252453484
    seeds, maps = parse_inputs(x)
    min_val = None
    min_arg = None
    number = 3974775726
    range_to_check = 100
    step = range_to_check // 1000
    for i in range(number - range_to_check, number + range_to_check, max(1, step)):
    # for i in seeds:
        val = traverse(i, maps)
        if min_val == None or min(val, min_val) == val:
            min_val = val
...
def f3(x):
    ...
    seeds, maps = parse_inputs(x)
    while True:
        x = traverse_reverse(i, maps, seeds)
        # print(i, ":", x)
        for j in range(len(seeds) // 2):
            if in_range(x, [seeds[j*2], seeds[j*2] + seeds[j*2 + 1]]):
                return i
        i += 1
        if i % 10000 == 0:
            print(i)

def find_input_seeds(seeds, start, end):
    for i in range(len(seeds) // 2):
        if overlaps(seeds, [start, end]):
            if in_range(start, [seeds, seeds + start - end]):
                return start
            if in_range(seeds, [start, end]):
                return seeds

def find_min(map, maps, seeds):
    outstart, instart, span = map
    if map_level == len(maps):
        found = find_input_seeds(seeds, start, end)
...
def f4(x):
    ...
    maps = maps.reverse()
    last_map = maps[0]
    last_map.sort(key=lambda x: x[0])
    for map in last_map:
        found = find_min(map, maps, seeds, 1)
        if found:
            return found

def convert_seeds(seeds, num_range):
    print("?", seeds, num_range)
    for i in range(len(seeds) // 2):
        if overlaps((seeds[i*2], seeds[i*2 + 1] + seeds[i*2]), (num_range[1], num_range[1] + num_range[2])):
            if in_range(seeds[i*2], (num_range[1], num_range[1] + num_range[2])):
                return seeds[i*2]
            else:
                return num_range[1]
        else:
...
# def get_input(line, depth, maps, seeds):
#     if depth == len(maps) and convert_seeds(seeds, (line[1], line[1] + line[2])) != False:
#         return traverse(convert_seeds(seeds, (line[1], line[1] + line[2])))
#     else:
#         pass
...
def g(x):
    ...
    # print(traverse(2149186375, maps))
    ...
    for map in maps:
        ...
        for line in map:
            if line[0] != lowest:
                sections.append([lowest, lowest, line[0] - lowest])
                sections.append(line)
            else:
                sections.append(line)
            lowest = line[0] + line[2]
        new_maps.append(sections)
    # print(new_maps)

    def get_all_divisions(target, map):
        print("input", target)
        divisions = []
        cursor = target[1]
        left = target[2]
        for section in map:
            if in_range(cursor, (section[0], section[0] + section[2])):
                section_r = min(section[2], left)
                if left != 0:
                    divisions.append([cursor, cursor - section[0] + section[1], section_r])
                    if section_r == left:
                        print("!!", divisions, section)
                        return divisions
                    else:
                        left -= section[2]
                        cursor += section[2]
                    assert left > 0
        if left != 0:
            divisions.append([cursor, cursor, left])
        print("!!!", divisions)
        return divisions
    
    def recurse(target, maps):
        if len(maps) == 0:
            gold = convert_seeds(seeds, target)
            return gold
        
        divisions = get_all_divisions(target, maps[-1])
        for division in divisions:
            solution = recurse(division, maps[:-1])
            if solution:
                return solution
        return False
    
    for section in new_maps[-1]:
        print("&&", section)
        solution = recurse(section, new_maps[:-1])
        if solution:
            print(":o", solution)
            return traverse(solution, new_maps)
...
seeds, maps = parse_inputs(e)
# print("===", traverse(79, maps))
# print("===", traverse(14, maps))
# print("===", traverse(55, maps))
# print("===", traverse(13, maps))
