...
pattern = re.compile(r"(?P<pn>[0-9]+)", re.IGNORECASE)

def get_int_match(match, name):
    return int(match.group(name))

def parse_line(col, l, maxlines):
    sds = []
    maxj = len(l)
    search_l = l
    while not pattern.search(search_l) == None:
        sd = []
        start, end = pattern.search(search_l).span()
        for i in range(col-1, col+2):
            for j in range(start-1, end + 1):
                if (i == col and (j != start - 1 and j != end)):
                    # print(i, j, end)
                    continue
                elif (i < 0 or j < 0):
                    continue
                elif (i >= maxlines or j >= maxj):
                    continue
                sd.append((i, j))
        sds.append((pattern.search(search_l).group("pn"), sd))
        search_l = search_l[end:]
    return sds

def parse_line2(col, l, maxlines):
    # nums = pattern.findall(l)
    sds = []
    maxj = len(l)
    for num in pattern.finditer(l):
        start, end = num.span()
        sd = []
        # print(start, end, num)
        for i in range(col - 1, col + 2):
            for j in range(start - 1, end + 1):
                if (i == col and (j != start - 1 and j != end)):
                    ...
                    continue
                sd.append((i, j))
        sds.append((int(num.group("pn")), sd))
    return sds
...
def f(x):
    ...
    for i, line in enumerate(x):
        # print(line)
        sds = parse_line2(i, line, len(x))
        for num, sd in sds:
            # print(num, sd)
            found = False
            for coord in sd:
                if x[coord[0]][coord[1]] not in ".1234567890" and not found:
                    count += int(num)
                    num_found += 1
                    # if num_found < 10:
                        # print(num, coord, line, x[coord[0]][coord[1]])
                    found = True
    return count
...
def f2(x):
    ...
    for i, line in enumerate(x):
        ...
        for num, sd in sds:
            ...
            for row, col in sd:
                if x[row][col] == "*":
                    if (row, col) in geardict:
                        geardict[(row, col)].append(int(num))
                    else:
                        geardict[(row, col)] = [int(num)]
    # print(geardict)
    for gearinfo in geardict.values():
        if len(gearinfo) == 2:
            num_found += 1
            gearratio = gearinfo[0] * gearinfo[1]
            count += gearratio
            # print(gearinfo)
