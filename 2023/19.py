...
def f2(x):
    workflows_str, parts = x.strip("\n").split("\n\n")
    workflows_str = workflows_str.splitlines()
    workflows = dict()
    accepted_goals = []
    for workflow in workflows_str:
        label, rest = workflow.split("{")
        rest = rest[:-1]
        steps = rest.split(",")
        steps_list = []
        for step in steps[:-1]:
            assert ":" in step
            cond, dest = step.split(":")
            steps_list.append((cond, dest))
        steps_list.append((True, steps[:-1]))
        workflows[label] = steps_list
