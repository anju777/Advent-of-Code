...
def f(x):
    ...
    pmap = []
    seen = set()
    for linei, line in enumerate(x):
        pmap.append([list(z) for z in zip(list(line), [-1] * len(line))])
        if "S" in line:
            start = (linei, line.index("S"))
            seen.add(start)
    
    pmap[start[0]][start[1]][1] = 0
    curr_spots = [start]
    curr_steps = 0
    
    def is_valid(adjacent):
        adjr, adjc = adjacent
        if adjr < 0 or adjc < 0:
            return False
        elif adjr >= len(pmap) or adjc >= len(pmap[0]):
            return False
        return True

    def is_connected(spot1, spot2):
        r1, c1 = spot1
        r2, c2 = spot2
        
        # Spot 1 always on left
        if c1 > c2:
            r2, c2 = spot1
            r1, c1 = spot2
        # Spot 1 always on top
        if r1 > r2:
            r2, c2 = spot1
            r1, c1 = spot2
        ...
        if horizontal:
            if key1 == "-" or key1 == "S":
                return key2 == "-" or key2 == "J" or key2 == "7"
            if key1 == "L" or key1 == "F":
                return key2 == "-" or key2 == "J" or key2 == "7" or key2 == "S"
            return False
        else:
            # Vertical
            if key1 == "|" or key1 == "S":
                return key2 == "|" or key2 == "L" or key2 == "J"
            elif key1 == "7" or key1 == "F":
                return key2 == "|" or key2 == "L" or key2 == "J" or key2 == "S"
            return False
    ...
    while True:
        ...
        for spot in curr_spots:
            currx, curry = spot
            adjacents = [(currx - 1, curry), (currx + 1, curry), (currx, curry - 1), (currx, curry + 1)]
            for adjacent in adjacents:
                if is_valid(adjacent) and adjacent not in seen and is_connected(adjacent, spot):
                    # print(is_connected(adjacent, spot))
                    pmap[adjacent[0]][adjacent[1]][1] = curr_steps
                    placed = True
                    seen.add(adjacent)
                    new_curr_spots.append(adjacent)
        curr_spots = new_curr_spots
        ...
        if not placed:
            ...
            # pprint.pprint(pmap)
            for row in pmap:
                maxval = max(max(row, key=lambda x: x[1])[1], maxval)
            return maxval
    pass
...
def f2(x):
    ...
    for linei, line in enumerate(x):
        ...
        maps.append(line)
        if "S" in line:
            start = (linei * 2, line.index("S"))
            seen.add(start)
        filler_line = list(["|"] * len(line))
        pmap.append([list(z) for z in zip(filler_line, [-1] * len(line))])
        maps.append("".join(filler_line))
    ...
    while True:
        ...
        if not placed:
            maxval = 0
            for row in pmap:
                maxval = max(maxval, max(row, key=lambda x: x[1])[1])
            print("maxval", maxval)
            contenders = []
            seenc = set()
            for row in range((len(pmap) + 1) // 2):
                for col in range((len(pmap[0]) + 1) // 2):
                    if pmap[row * 2][col * 2][1] == -1:
                        contenders.append((row * 2, col * 2))
                # contenders.extend(list(filter(lambda x: x[1] == -1, row)))
            # print(contenders)
            # pprint.pprint(pmap)
            ...
            for contender in contenders:
                count += 1
                if count % 100 == 0:
                    print(f"{count}/{len(contenders)}")
                seencontender = set()
                if contender in seenc or contender in escaped:
                    continue

                checks = [contender]
                know = False
                made_move = True
                while not know and made_move:
                    # print("Here!", contender)
                    made_move = False
                    new_checks = []
                    for check in checks:
                        seencontender.add(check)
                        seenc.add(check)
                        currx, curry = check
                        adjacents = [("up", (currx - 1, curry)), ("down", (currx + 1, curry)), ("left",(currx, curry - 1)), ("right",(currx, curry + 1))]
                        for dire, adjacent in adjacents:
                            if not is_valid(adjacent):
                                # print("Escaped!", adjacent, seencontender)
                                escaped.add(contender)
                                escaped = escaped.union(seencontender)

                                know = True
                                break
                            elif pmap[adjacent[0]][adjacent[1]][1] != -1:
                                pass
                            elif adjacent in escaped:
                                escaped.add(contender)
                                escaped = escaped.union(seencontender)
                                know = True
                                break
                            elif adjacent not in seenc:
                                made_move = True
                                new_checks.append(adjacent)
                            seencontender.add(adjacent)
                            seenc.add(adjacent)
                        if know:
                            break
                    checks = new_checks
            ...
            # print(contenders)
            # print("----------")
            # print(len(escaped.intersection(contender)))
            # print(set(contenders))
            # print(set(contenders).difference(escaped))
            return len(contenders) - len(escaped.intersection(contenders))
    pass
